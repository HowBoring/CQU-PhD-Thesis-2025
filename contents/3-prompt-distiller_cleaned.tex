...
本章聚焦于深度模型数据与知识获取环节的效率问题。针对少样本场景下，知识蒸馏存在的迁移不稳定与过拟合问题，提出 Prompt-Distiller 方法，通过双教师协同蒸馏与基于对比学习的中间层知识对齐，在一定程度上提升提示微调模型在少样本条件下的蒸馏效果与泛化能力。该方法为后续章节的小模型高效构建提供了数据与知识层面的基础支撑。本章首先介绍了知识蒸馏在少样本场景下面临的挑战，随后阐述了 Prompt-Distiller 方法的设计思路与实现细节，最后通过实验验证了该方法的有效性与适用性。
...
\textbf{步骤5：输出学生模型。} 当训练结束后，得到更新后的学生模型参数 $\Theta_S^*$。这即是知识蒸馏后的最终模型，可用于后续章节的系统部署。由于学生模型通常比教师模型小一个量级以上，其推理速度和资源占用有所降低，而性能与教师模型相近。本章稍后通过实验展示 Prompt-Distiller 训练得到的学生模型在多个任务上的结果。
