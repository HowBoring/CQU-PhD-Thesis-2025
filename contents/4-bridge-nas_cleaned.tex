...
在延续前文对深度模型构建核心环节与效率瓶颈的系统梳理及理论准备的基础上，本文首先聚焦于架构设计与评估这一在早期模型范式中最突出的效率瓶颈。当以 CNN 等为代表的架构工程主导模型性能提升时，如何以更低的计算与人工代价获得高质量架构，成为高效构建的首要挑战。然而，传统 NAS 方法普遍存在算力消耗巨大、架构经验难以跨搜索空间复用等问题，进一步加剧了这一矛盾。针对上述问题，本章将重点探讨跨异构搜索空间的架构知识迁移机制，并提出一种基于表征学习的可迁移 NAS 框架，以降低搜索与评估成本、提升架构先验的可复用性，从而实现高效与智能化的模型架构优化。

\mysection{引言}
\label{sec:ch4-1-introduction}

在深度学习的发展历程中，模型架构的演进始终是推动性能上限攀升与能力边界扩展的核心力量。无论是从早期的 CNN 到 ResNet，还是自注意力与Transformer所引发的范式转移，性能的跃迁往往首先体现在架构层面对表达力与归纳偏置的重塑。模型架构（亦称神经架构）定义了参数承载的运算流，是学习算法与数据分布之间的接口；它既规定了信息如何被逐层变换，也隐含了模型在样本复杂性、尺度不变性、长程依赖等方面的先验。形式化地说，模型架构可抽象为一个有向无环图（DAG），其中结点对应算子（卷积、注意力、归一化、激活等），边则刻画张量在算子之间的流动与复用关系。实际设计中，研究者往往需要在宏观层面的网络深宽、模块堆叠与跨层连接，和微观层面的算子类型、超参数与融合顺序之间做权衡；再叠加训练策略与数据分布，设计空间呈组合爆炸，手工探索成本高昂且难以保证全局最优。这直接催生了 NAS 作为自动化架构设计的重要研究方向。

早期 NAS 的主要瓶颈在于评估：候选架构的优劣需要通过从零训练获取验证集性能来判别，强化学习与演化算法等黑箱优化在大规模候选集上迭代评估，导致训练开销与计算预算成数量级增长。围绕这一评估成本瓶颈，研究逐步发展出一系列高效化技术路径。其中，权重共享方法通过在统一超网内共享边或算子的参数，使不同子架构的性能可以在一次训练后快速近似评估，从而将多次完全训练的成本压缩为一次代价可控的超网训练\cite{efficientneuralarchitecture_pham_2018}。低保真评估则通过缩短训练轮次、降低分辨率与模型宽度、使用子集数据或启用早停与学习曲线外推等代理手段，以牺牲一定评估精度换取搜索速度\cite{cellbasedfast_dong_2023}。此外，梯度化方法将离散的架构选择通过连续化松弛映射为可微参数，利用双层优化在训练权重与架构参数之间交替更新，从而以一阶或二阶梯度近似显著降低搜索时间\cite{dartsdifferentiablearchitecture_liu_2019}。尽管该思路带来了优化稳定性与离散化落差等问题，但在大规模搜索空间上的效率优势依然突出。与此同时，基于预测器的方案以学习式替代昂贵的训练—评估环节。通过将架构编码为序列或图表征，训练性能预测模型来估计未训练架构的准确率、收敛速度或硬件延迟，进而用作目标或约束指导搜索\cite{peepholepredictingnetwork_deng_2017,surrogateassistedevolutionary_sun_2020,neuralpredictorneural_wen_2020,renasrelativisticevaluation_xu_2021}。

为了提升 NAS 的效率，另一条重要路径是让经验可迁移。TNAS旨在跨任务复用以往搜索或训练积累，从而将一次性高昂搜索成本摊销到多任务场景。其核心在于学习一个能将“架构–任务–性能”三元关系对齐的表征与归纳机制。一方面需要对架构进行结构化编码，使得相似拓扑与算子组合在嵌入空间中相邻，另一方面需引入任务侧先验以进行条件化预测与迁移。以 Arch-Graph~\cite{archgraphacyclic_huang_2022} 为代表的方法将架构视为图结构，利用图神经网络在架构的计算图上学习到可泛化的结构嵌入，并在不同任务之间传播性能线索，从而在新任务上以较少评估样本实现冷启动加速。EMT-NAS~\cite{emtnastransferring_liao_2023} 等多任务/多目标取向的方法则借助演化多任务优化或参数化超网，将不同任务的搜索过程联结为共享表征与部分权重的协同演化，通过知识迁移与负迁移抑制同时优化准确率与延迟、能耗等硬件指标。总体而言，TNAS 将搜索从一次性工程转化为可复用、可累积的能力库，在部分场景中表现出较好的性价比。然而，现有 TNAS 的研究成果大多局限于同构搜索空间内的知识迁移。本章的核心挑战在于突破这一限制，实现跨越不同架构范式和定义的异构搜索空间之间的有效知识迁移。

目前，直接探讨跨异构搜索空间 NAS 的研究尚处于起步阶段，相关工作非常有限 。Liu 等人提出的跨域预测器（Cross-Domain Predictor，CDP）\cite{bridgegaparchitecture_liu_2022} 是少数直接尝试应对此挑战的工作之一 。CDP 的核心思想并非直接迁移架构本身，而是试图训练一个能够在源域和目标域之间通用的性能预测模型，而不限于同构的架构搜索空间。它利用领域自适应技术，将源空间和目标空间的架构投射到一个共享的特征空间，并在此空间训练性能预测器。其目标是实现跨空间的性能度量，从而在目标域 NAS 中，能够借助源域的性能数据来辅助评估，提高搜索效率。然而，CDP 方法存在明显局限。首先，它仍依赖于源、目标空间在表征上具有一定的可比性或相似性，以便特征能够有效对齐；其次，CDP 仅提供了一个性能评估工具，并未给出如何将源域的优秀架构直接迁移或转化为目标域候选解的方案，目标空间的架构仍需独立搜索产生。因此，CDP 更像是 NAS 的一种辅助加速手段，而非完整的跨异构迁移解决方案。

除了 CDP 之外，鲜有文献直接研究如何在不同架构范式之间进行设计经验的显式迁移 。这一研究空白凸显了当前领域面临的挑战。要实现跨异构空间的架构知识迁移，一个关键的前提是需要一种能够统一编码不同类型架构的机制。不同的搜索空间往往采用截然不同的设计语言来描述架构，包括不同的基础算子集合、不同的连接拓扑规则、以及不同的编码方式。这导致源域中的一个架构 $a^{(S)}$ 无法直接翻译或映射为目标域中的一个有效架构 $a^{(T)}$。缺乏统一的表征框架，使得跨越异构空间的架构比较、关联和知识传递变得极其困难。近年来，神经架构表征学习的研究为此提供了有前景的方向。其核心目标是学习一个从离散的、图状的架构空间到连续的、低维向量空间的映射（即编码器），使得架构在潜在表征空间中能够被方便地比较、插值和操作。例如，有工作提出将神经网络架构视为图结构，并利用图神经网络或 Transformer 等模型来学习其向量嵌入~\cite{neuralgraphembedding_li_2020}。Lukasik 等人提出的平滑变分图嵌入方法，利用变分自编码器将架构嵌入到向量空间，并在此空间进行高效优化 \cite{smoothvariationalgraph_lukasik_2021}。Yan 等人则研究了无监督的架构表征学习，证明了良好的表征有助于预测架构性能并加速 NAS~\cite{doesunsupervisedarchitecture_yan_2020}。这些工作表明，通过学习一个恰当的、能够捕捉架构关键结构与语义信息的潜在表征空间，有可能弥合不同搜索空间之间的语言障碍。其次，即使能够建立统一表征，跨域性能关联的不确定性也构成了严峻挑战 。一个在源域任务和架构空间中表现优异的架构，迁移到目标域后其性能如何，往往难以直接预测。任务目标、数据集特性以及架构空间本身的差异都可能导致性能关联发生变化。简单地将在源域中性能最优的架构直接迁移至目标域，未必能获得最佳结果。最后，直接或生硬的架构迁移极易引发负迁移的风险。由于源域和目标域的最优架构可能存在结构上的显著差异，从源域迁移过来的架构在目标域中可能并非最优，甚至性能不佳。若迁移策略过于僵化，强行将源域知识施加于目标域搜索，反而可能限制了对目标域更优架构的探索。

本文提出的基于表征构建的域间异构演化 TNAS（Building Representation for Inter-Domain Heterogeneous Evolutionary TNAS，\textsc{Bridge}）框架正是在这一背景下，试图将统一的架构表征学习与显式的跨域解迁移相结合，以解决跨异构 TNAS 的核心难题。与 CDP 仅关注性能预测不同，\textsc{Bridge} 旨在学习一个共享的潜在表征空间，并在该空间中学习跨域映射函数，从而能够将源域的优秀架构解直接转化为目标域的高质量初始候选。进一步地，通过将这种显式迁移的解融入演化优化框架，\textsc{Bridge} 实现了知识利用与适应性探索的结合。因此，相较于现有有限的探索，\textsc{Bridge} 提供了一种更完整、更直接的跨异构搜索空间神经架构知识迁移解决方案。本章的主要贡献如下：
\begin{enumerate}
	\item 提出了一种面向跨异构搜索空间的演化式TNAS框架。该框架旨在解决已有架构设计经验难以在异构搜索空间之间迁移复用的核心效率瓶颈。
	\item 提出了一种用于跨空间TNAS的神经架构表征学习方法。该方法通过设计统一的神经架构序列化方案以适配异构空间；进而，构建了一种结合Transformer与变分自编码器（Variational Autoencoder，VAE）的表征模型，用以提取架构关键特征并构建平滑的潜在表征空间。此外，该方法引入架构性能指标作为监督信号，以优化潜在空间的分布，使后续的迁移搜索过程可由性能指标驱动。
	\item 提出了一种演化式贯序迁移优化（Evolutionary Sequential Transfer Optimization，ESTO）方法。该方法利用前述神经表征学习所构建的跨域表征映射能力，将源搜索空间中的已有NAS解高效迁移至目标搜索空间，并将其作为演化算法的初始种群，从而显著加速搜索过程。实证评估表明，该方法在多个不同规模的异构搜索空间中，均取得了显著的搜索效率与模型性能提升。
\end{enumerate}
...
为此，\textsc{Bridge} 并非直接迁移原始架构结构，而是通过学习架构的语义表征，并在表征空间中建立跨域映射，实现语义对齐而非结构复制的迁移范式。

首先对整体框架进行宏观概述，随后逐层深入剖析其三大核心组件：
（1）神经架构表征的构建：通过定制化分词器与基于 Transformer 的变分自编码器，将图结构的神经架构编码为连续、结构感知且性能可预测的隐向量；
（2）跨域表征映射的构造：在源域与目标域的表征空间之间学习一个可微或可优化的映射函数，使得源域中的高性能架构在目标域中具有语义等价的对应；
（3）基于跨域顺序迁移的演化 TNAS：将迁移后的架构作为初始种群，引导目标域上的演化搜索过程，兼顾探索与利用，有效规避负迁移风险。这些组件协同工作，共同构成了一个通用、可扩展且任务自适应的迁移神经架构搜索 TNAS 系统。

如图~\ref{fig:overview} 所示，\textsc{Bridge} 的工作流程遵循“表征–映射–迁移搜索”的步骤。
首先，在表征学习阶段，分别在源域 $\mathcal{S}$ 与目标域 $\mathcal{T}$ 上独立训练神经架构表征学习器。
该学习器包含编码器、解码器与性能预测器三个模块，其目标是建立从离散架构空间到连续表征空间的双向映射，并确保该表征空间与架构性能高度相关。
...
\begin{figure}[t!]
	...
	\bicaption[\textsc{Bridge}框架概览]{
		...
		整个流程可分为三个关键阶段：
		(1) 通过有监督与无监督表征学习构建神经架构的紧凑且语义丰富的隐式表征；
		(2) 在源域与目标域的表征空间之间学习一个结构保持的映射函数，以对齐不同搜索空间下的架构语义；
		(3) 利用所学表征模型与跨域映射，将源域中历史搜索获得的高性能架构解迁移至目标域，作为演化搜索的高质量初始种群，从而在一定程度上加速目标域的神经架构搜索过程。
...
随后，在映射学习阶段，利用少量在目标域上评估过的架构样本（通常仅需数百个），结合源域中已知的高性能架构，构建一个从源域表征空间 $\mathbb{R}_{\mathcal{S}}$ 到目标域表征空间 $\mathbb{R}_{\mathcal{T}}$ 的映射函数 $\mathcal{M}$。
...
该种群作为演化算法的起点，缩小了目标域的搜索空间，使搜索过程能够更快地收敛至高性能区域。
整个框架的设计哲学在于：不依赖源域与目标域结构相似性，而是通过语义表征实现跨域知识的解耦与重用。
...
为减少信息损失，特别是配合基于 Transformer 的表征学习器，
...
该定制化策略确保分词器能够有效捕捉并表征不同神经架构的复杂特性。
...
其对应序列将被无缝拼接，从而在统一序列编码中完整保留各类单元的独特特性。
...
\begin{figure*}[t]
	...
	\bicaption[表征学习器示意图]{
		所提表征学习器的示意图。
		在预训练阶段，仅优化神经架构重建损失，相对性能预测器不参与训练；
		在微调阶段，重建损失与预测损失联合优化。
...
\end{align}
其中 $ \bm{m} $ 表示输入架构的记忆，
$ \bm{r}_{\mu} $ 与 $ \bm{r}_{\sigma} $ 分别为所学神经架构表征分布的均值与方差。
...
此处的主要目标是利用架构的性能度量建立不同搜索空间对应表征空间之间的映射，
从而促进跨域解的迁移，提升搜索效率与效果。
为增强神经架构表征的学习过程，本研究引入与架构性能相关的有监督信号。
具体而言，本研究采用神经架构性能预测器快速评估架构或其表征。
该预测器用于调整架构表征的分布，使其更聚焦于性能指标。
此外，预测器还用于构建表征空间间的映射，
通过高效提供期望的性能标签，使模型更好地理解架构表征与其性能之间的关系，
从而引导搜索过程朝向高性能解。
...
神经架构表征的构建包含无监督预训练与有监督微调两个阶段，
二者目标不同但互补，共同实现鲁棒且可迁移的表征。

在预训练阶段，表征学习器通过编码与解码处理源域与目标域的神经架构，
建立对架构模式与特征的基础理解，且不依赖性能指标。
随后的微调阶段则引入任务特定的性能数据，
在保持可迁移性的同时优化目标域表征。
该监督阶段校准所学表征，使其与领域特定需求及性能目标对齐。
...
在框架的预训练阶段，一个特点是相对性能预测器（排序器）刻意保持不活跃。
该组件在此阶段不参与训练。
本研究采用优化目标解耦策略，
...
基于前述架构表征学习器的组件，NAS 解可通过所提 \textsc{Bridge} 显式迁移至其他领域的任务。
如图~\ref{fig:overview} 所示，以下步骤聚焦于构建源域与目标域表征空间之间的映射，
实现神经架构解的显式跨域迁移。
通过建立该映射，模型可有效利用源域知识提升目标域的搜索效率，发现高性能架构。

构建源域与目标域表征空间间的映射对跨域知识迁移至关重要。
令 $ \mathbb{R}^{\mathstrut}_{\mathcal{S}} $ 与 $ \mathbb{R}^{\mathstrut}_{\mathcal{T}} $ 分别表示源域（$ \mathcal{S} $）与目标域（$ \mathcal{T} $）的表征空间。
目标是学习映射函数 $ \mathcal{M}: \mathbb{R}^{\mathstrut}_{\mathcal{S}} \rightarrow \mathbb{R}^{\mathstrut}_\mathcal{T} $，
最小化源域映射表征与目标域对应表征间的差异。
形式化地，待最小化的目标函数为：
\begin{equation}
	\mathcal{L}_\mathrm{map}(\mathcal{M}) = \sum_{i=1}^{N}{\ell\left(\mathcal{M}(\bm{z}_{s}^{(i)}),\bm{z}_{t}^{(i)}\right)}
\end{equation}
其中 $ \bm{z}^{(i)}_{s} \in \mathbb{R}^{\mathstrut}_{\mathcal{S}} $ 与 $ \bm{z}^{(i)}_{t} \in \mathbb{R}^{\mathstrut}_{\mathcal{T}} $ 分别为按预测值排序后第 $ i $ 个采样表征，
$ N $ 为采样表征的总数，$ \ell $ 为衡量映射源表征与目标表征差异的损失函数。
映射函数 $ \mathcal{M} $ 的选择取决于表征空间特性及所需灵活性与复杂度。
本文以线性映射为例：$ \mathcal{M}(\bm{z}_{s}) = \bm{z}_{s}W + \bm{b} $，
其中 $ W $ 与 $ \bm{b} $ 为可学习的权重矩阵与偏置向量。
采用均方误差作为 $ \ell $，映射通过优化下式学习将源域表征投影至目标域：
\begin{equation}
...
\begin{itemize}[leftmargin=3\ccwd]
	\item \textbf{知识编码（第 1–2 行）：} 从源域最优种群中选取的架构通过源域编码器编码为表征向量，捕获源域中发现的高性能架构本质。
	\item \textbf{跨域映射（第 3 行）：} 为将知识迁移至目标域，编码表征通过所学映射函数 $ \mathcal{M} $ 从源域表征空间映射至目标域表征空间，有效将源域最优解投影至目标域架构搜索空间。
	\item \textbf{初始种群播种（第 4–6 行）：} 目标域中的映射表征作为演化搜索的初始种群，该种群以迁移知识为种子，为目标域架构搜索空间探索提供战略性起点。
...
迁移学习中的一个关键挑战是负迁移，指迁移知识与目标任务需求的根本性不匹配。
...
在上述的操作定义中，\texttt{CONV} 表示卷积操作，\texttt{SEP-CONV} 表示深度可分离卷积，\texttt{DIL-CONV} 表示空洞卷积，数字（如 \texttt{3X3}）表示卷积核的尺寸。\texttt{MAX-POOL} 表示最大池化，\texttt{AVG-POOL} 表示平均池化，数字（如 \texttt{3X3}）表示池化核的尺寸。\texttt{SKIP-CONNECT} 表示跳跃连接，\texttt{NONE} 表示无操作，即断路。
...
下半部分展示了使用 Kendall's \( \tau \)-b 相关系数（K-Tau）进行排序器微调的结果。该方法在 NAS-Bench-101 和 NAS-Bench-201 上分别达到 0.848 和 0.827，优于 NAO~\cite{peepholepredictingnetwork_deng_2017}、TNASP~\cite{endendperformance_sun_2023} 和 ReNAS~\cite{renasrelativisticevaluation_xu_2021}。在 DARTS 上，结果达到 0.811，支持有效的架构迁移。这些结果表明该排序预测器与实际性能排序具有更强的一致性。
...
\begin{figure}[t!]
	...
	\bicaption[跨域ESTO性能对比]{在 NAS-Bench-101 上，有无 \textsc{Bridge} 跨域 ESTO 机制的演化神经架构搜索性能对比分析。
		\ 上图： 每代探索架构的真实准确率变化趋势。
		\ 下图： 排序模块预测的相对适应度分数，用于指导 NAS 过程作为评估策略。
		...
		\ Top: evolution of true accuracy for architectures explored in each generation.
		\ Bottom: relative fitness scores predicted by the ranking module that guide the NAS process as the evaluation strategy.
...
本研究选取遗传算法（Genetic Algorithm，GA）~\cite{geneticalgorithms_holland_1992} 作为演化搜索求解器，该方法在 NAS 中被广泛使用。参照~\cite{efficienttwostage_hou_2021} 的设置，GA 求解器采用种群规模 20，使用单点交叉和多项式变异（概率均为 0.5），以及（Binary Tournament Selection）。
...
为严格验证这种通用性，表~\ref{tab:transfer-result-101} 中展示了与两种先进演化方法的集成实验：来自 EPCNAS~\cite{splitlevelevolutionary_huang_2023} 的粒子群优化（PSO）和来自 MSNAS~\cite{cellbasedfast_dong_2023} 的模因算法（MA）。实验结果表明，PSO 和 MA 分别比其基线实现提升了 0.19\% 和 0.25\%。重要的是，这些提升是在减少计算开销（更少的评估次数和迭代轮次）的前提下实现的。这些发现为框架的求解器无关性及其增强多种演化优化策略的能力提供了有力实证。
...
表~\ref{tab:transfer-result-301} 总结了所提出方法与基线方法的性能对比。诸如 DARTS（97.24\%，耗时 4 GPU 日）和 MSNAS（97.32\%，耗时 0.23 GPU 日）等方法表明，高效的搜索机制可显著降低资源消耗。当引入迁移学习后（如 EMT-NAS 和 MTNAS，均采用 EA\,+\,TL 策略），模型在性能和成本上均获得进一步增益。

所提出的方法 \textsc{Bridge} 同样采用 EA\,+\,TL 策略，在仅 0.21 GPU 日的搜索成本下达到了最高的准确率（97.33\%\,\(\pm\)\,0.06）。该结果不仅优于朴素 GA 基线（96.62\%，0.3 GPU 日），还在 DARTS 搜索空间中通过将高质量解从简单域有效迁移到复杂域，树立了新基准。
...
\begin{table}
	...
	\small\begin{NiceTabularX}{\textwidth}{p{9em}X[c]X[c]l}[notes,tabularnote={EA = 演化算法, RL = 强化学习, GD = 基于梯度, TL = 迁移学习。}]
		...
		\toprule
		方法                                             & \Block{1-1}{平均准确率~(\%)} & \Block{1-1}{搜索成本 (GPU 日)} & \Block{1-1}{方法类型} \\
		\midrule\midrule
...
此外，将架构种群的演化动态映射到潜在空间中，如图~\ref{fig:nb101-mds-trace} 所示，说明迁移学习如何加速演化过程。通过利用 \textsc{Bridge}，以高质量且多样化的解初始化种群，显著提升收敛速度，并发现先前未见的高性能解。
...
如第~\ref{sec:ch4-5-4-mixed-supervision-training} 节所述，所提出的表征学习框架采用两阶段训练策略：无监督预训练后接混合监督微调。该策略既能全面捕捉架构特性，又保持跨域可迁移性。表征学习模型作为所提迁移框架的核心基础，其所学表征的质量对知识迁移的有效性与最终任务性能具有决定性影响。鉴于表征质量在很大程度上依赖于训练数据的规模与分布，本文继而开展了一项全面的实证研究，旨在系统性地探究数据集规模对表征学习性能的影响。此项分析为评估所提方法在不同数据规模条件下的可扩展性与鲁棒性，提供了关键的实证支撑。

表~\ref{tab:ablation-pretrain-data-size} 展示了 NAS-Bench-101 数据集不同比例下预训练阶段的关键性能指标，特别分析了两个核心指标：VAE 重建准确率和计算开销（以 GPU 日计）。实验结果揭示了架构表征学习中的两个关键模式：1）模型性能与训练数据量呈强正相关，10\% 数据量时 VAE 准确率约 99.91\%，且计算开销极低（0.08 GPU 日）；2）超过该阈值后，当数据量增至 30\% 时，准确率变化不显著（约 99.56\%，略有下降），但计算成本上升 37.5\%。这些发现表明，即使在有限数据条件下，该框架也能学习到鲁棒的架构嵌入。这种在相对小数据集下的高效表征学习能力，为实际迁移学习应用（尤其在架构数据难以获取或计算成本高昂的场景）提供了坚实的实证基础。
...
\begin{table}
	...
	\small\begin{NiceTabularX}{\linewidth}{p{12em}*{5}{>{\centering\arraybackslash}X}}
		...
		\textbf{成本} (GPU 日)     & 0.08          & 0.08          & 0.08           & 0.11           & 0.15           \\
		\bottomrule
	\end{NiceTabularX}
\end{table}
...
为评估这些超参数对性能的影响，采用两个互补指标：相关性与重要性。相关性通过 Pearson 相关系数量化超参数与性能间的线性关系；重要性则通过训练随机森林回归器评估，以超参数为输入特征、性能指标为目标变量，其特征重要性得分揭示了非线性关系。
...
\textsc{Bridge} 框架的核心创新在于其解决异构迁移难题的独特思路：它并非直接迁移架构的句法结构，而是通过引入统一的神经架构表征学习机制，将来自不同搜索空间（具有不同操作集、拓扑规则和编码方式）的架构映射到结构感知的潜在语义空间。在此基础上，框架进一步学习了一个跨域表征映射函数，以显式地桥接源域与目标域之间的表征差异，使得源域的高性能架构解能够在目标域中找到语义上对应的表征。最终，通过将这些迁移而来的高质量表征解码为目标域架构，并将其作为演化顺序迁移优化策略的初始种群，\textsc{Bridge} 实现了源域知识的高效引导与目标域架构的自适应演化相结合。

本章详细介绍了 \textsc{Bridge} 框架的各个关键技术组件，包括定制化的神经架构分词器、基于 Transformer 的变分自编码器（用于表征学习）、基于性能排序的映射学习方法以及 ESTO 算法的具体实现。通过在一系列复杂度递增的标准 NAS 基准（NAS-Bench-101, NAS-Bench-201, DARTS）上进行的大量实验验证，结果表明：(1) 所提出的架构表征学习方法能够有效地捕捉架构的关键特征并与性能相关联；(2) \textsc{Bridge} 框架能够成功地实现跨越显著异构的搜索空间（例如从 NAS-Bench-201 到 NAS-Bench-101，从 NAS-Bench-101 到 DARTS）的知识迁移；(3) 与不使用迁移的基线 NAS 方法或其他 TNAS 方法相比，\textsc{Bridge} 能够在显著降低搜索成本（例如，平均减少约 50\% 的 GPU 评估时间）的同时，发现性能相当甚至更优的神经网络架构。

综上所述，本章提出的 \textsc{Bridge} 框架为解决跨异构搜索空间的神经架构知识迁移这一前沿挑战提供了一种有效且可行的解决方案。通过结合先进的表征学习技术与演化迁移优化策略，\textsc{Bridge} 不仅显著提升了 NAS 的效率和通用性，也为自动化机器学习领域中更广泛的知识复用与迁移问题提供了新的思路和启示。
