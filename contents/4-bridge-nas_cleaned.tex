...
本章聚焦于深度模型结构设计环节的效率挑战，特别是针对神经架构搜索（NAS）计算成本高昂，以及架构知识难以跨越异构搜索空间进行复用的问题。为应对此问题，本章提出一种基于表征学习的跨空间可迁移神经架构搜索框架（\textsc{Bridge}）。该方法通过构建统一的架构表示学习机制与跨域表征映射，在不同架构范式间进行知识迁移。本章首先介绍 NAS 领域中知识迁移面临的挑战，随后阐述 \textsc{Bridge} 框架的总体设计、核心的架构表征学习与跨域映射方法，最后通过在多个基准上的实验验证该框架在搜索效率与架构发现方面的有效性。
...
\item 提出了一种用于跨空间TNAS的神经架构表示学习方法。该方法通过设计统一的神经架构序列化方案以适配异构空间；进而，构建了一种结合Transformer与变分自编码器（Variational Autoencoder，VAE）的表征模型，用以提取架构关键特征并构建平滑的潜在表征空间。此外，该方法引入架构性能指标作为监督信号，以优化潜在空间的分布，使后续的迁移搜索过程可由性能指标驱动。
\item 提出了一种演化式贯序迁移优化（Evolutionary Sequential Transfer Optimization，ESTO）方法。该方法利用前述神经表示学习所构建的跨域表征映射能力，将源搜索空间中的已有NAS解高效迁移至目标搜索空间，并将其作为演化算法的初始种群，从而显著加速搜索过程。实证评估表明，该方法在多个不同规模的异构搜索空间中，均取得了显著的搜索效率与模型性能提升。
\end{enumerate}
...
\begin{figure}[t!]
	...
	\bicaption[\textsc{Bridge}框架概览]{
		...
		(2) 在源域与目标域的表示空间之间学习一个结构保持的映射函数，以对齐不同搜索空间下的架构语义；
			(3) 利用所学表示模型与跨域映射，将源域中历史搜索获得的高性能架构解迁移至目标域，作为演化搜索的高质量初始种群，从而在一定程度上加速目标域的神经架构搜索过程。
...
\textsc{Bridge} 框架的核心创新在于其解决异构迁移难题的独特思路：它并非直接迁移架构的句法结构，而是通过引入统一的神经架构表示学习机制，将来自不同搜索空间（具有不同操作集、拓扑规则和编码方式）的架构映射到结构感知的潜在语义空间。在此基础上，框架进一步学习了一个跨域表示映射函数，以显式地桥接源域与目标域之间的表示差异，使得源域的高性能架构解能够在目标域中找到语义上对应的表示。最终，通过将这些迁移而来的高质量表示解码为目标域架构，并将其作为演化顺序迁移优化策略的初始种群，\textsc{Bridge} 实现了源域知识的高效引导与目标域架构的自适应演化相结合。
