...
% 在大语言模型成为基础设施、统一架构与海量语料已相对完备的背景下，构建效率的瓶颈进一步转移到参数实现与复用。深度学习的研究重点转向通过参数优化实现对知识与能力的。在此背景下，如何在更少训练甚至无需训练的条件下整合多源专家能力、避免参数冲突与性能折损，成为高效构建的关键。本章据此提出知识图谱引导的多形式优化模型融合框架，将复杂的融合问题拆解为可递进求解的子形式，在知识显式建模与演化求解的闭环中实现稳定高效的参数层迁移与整合。

在 LLM 逐渐演化为通用智能基础设施的背景下，统一架构与大规模语料资源已趋于完善，模型构建效率的瓶颈随之从结构、数据层面转移至参数实现与优化机制。当前，深度学习研究的重点正由单一任务性能提升转向通过参数优化实现多任务能力的协同与统一表达。在此趋势下，如何在有限甚至无需额外训练的条件下有效整合多源专家模型能力，避免参数冲突与性能折损，成为高效模型构建的关键挑战。基于此，本章提出一种知识图谱引导的多形式优化模型融合框架，将复杂的融合问题分解为可递进求解的子任务，并在知识显式建模与演化优化的闭环机制中，实现参数层面的稳定迁移与高效整合，从而为通用智能模型的可持续演化提供理论与方法支撑。
...
为缓解参数冲突问题，后续研究提出了一系列“改进型”的静态融合策略。这些方法在执行融合前，先对模型参数进行预处理。例如，TIES-Merging \cite{tiesmergingresolving_yadav_2023} 在融合前对权重进行选择性裁剪和符号对齐；DARE \cite{languagemodelsare_yu_2024} 则受 Dropout 启发，对参数增量进行随机稀疏化后再求平均。这些预处理步骤在一定程度上减轻了冲突，但其融合过程本质上仍依赖固定的启发式规则，而非针对特定目标进行自适应优化。
...
% 本节给出 KG-MFTO 的确定性理论框架：定义跨形式知识共享的动态知识图谱、边上统计量的确定更新规则，以及在该图谱上进行关系推理与可行解建议的图神经网络（GNN）结构与训练目标。知识图谱作为全局显式记忆，GNN 对其进行参数化推理，输出用于课程规划与求解热启动的量（性能预测 \(\hat y_f\)、权重建议 \(\hat{\boldsymbol{\alpha}}_f\)、协同矩阵 \(\hat S_f\)），二者在闭环中协同工作。

% \mysubsection{动态异构知识图谱建模}
% \label{sec:ch5-4-1-dynamic-heterogeneous-kg-modeling}

% 本章构建了一个动态异构图 $G=(V,E)$ 来表示模型融合问题中的多种关系。图谱包含两类节点$V=\{\text{模型}, \text{任务}\}$，以及两类边$E=\{\text{模型--模型}, \text{模型--任务}\}$，分别对应模型之间的关系和模型与任务之间的关系。

% \textbf{节点表示}：每个模型节点$M_i$（对应专家模型$M_i$）被赋予一个可训练的$d$维嵌入表示$\mathbf{h}^M_i \in \mathbb{R}^d$（随机初始化，并随 GNN 联合优化）；每个任务节点$\tau_j$ 使用固定的语义向量$\mathbf{h}^T_j \in \mathbb{R}^{d_T}$，该向量由预训练的 SimCSE 编码器\cite{gao2021simcse} 从任务描述文本或名称计算得到，并在全流程中保持冻结。

% \textbf{边关系及统计量}：知识图谱的边刻画了模型间以及模型与任务间的性能关联信息，并随优化过程动态更新。具体而言，
...
% 每条模型--任务边 $e(M_i, \tau_j)$ 记录模型 $M_i$ 在任务 $\tau_j$ 上的性能指标 $\pi_{ij}$。这些数据提供了每个模型在各任务上的擅长程度和稳定性。

% 知识图谱的边上统计量在每次完成一个形式评估后进行确定更新。假设当前求解了形式 $f=(\mathcal{M}_f,\mathcal{T}_f)$，得到融合配比$\boldsymbol{\alpha}_f$和该融合模型的验证集性能$y_f=\mathcal{F}(\mathrm{Merge}(\mathcal{M}_f,\boldsymbol{\alpha}_f), \mathcal{T}_f)$。首先，对于任意在 $\mathcal{M}_f$ 中共同出现的模型对 $(M_i, M_{i'})$，计算它们在该形式下的协同增益：
% \begin{equation}
% 	g_f(i,i') = \Big(y_f - \bar{y}^{\text{ref}}_f\Big) \cdot \psi(\alpha_{f,i}, \alpha_{f,i'}),\quad \psi(a,b)=2ab,
% \end{equation}
% 其中 $\bar{y}^{\text{ref}}_f$ 取 $\mathcal{M}_f$ 中单个最好模型在 $\mathcal{T}_f$ 上的得分。随后采用滑动平均来更新协同分数：
% \begin{equation}
% 	s_{ii'} \leftarrow (1-\lambda) s_{ii'} + \lambda g_f(i,i'),\quad 0<\lambda\ll 1.
% \end{equation}
% 通过持续积累不同形式中的协作增益$g_f(i,i')$，$s_{ii'}$ 将逐渐收敛到模型对 $(i,i')$ 在全局融合中的实际交互关系：若二者经常在一起出现在高性能融合解中，$s_{ii'}$ 会变为较大的正值；反之若二者共存时性能往往低于参考，$s_{ii'}$ 将趋向负值。

% 接下来，采用计数式退火更新模型对的不确定度 $u_{ii'}$：
% \begin{align}
% 	n_{ii'} & \leftarrow n_{ii'} + \mathbb{I}\{i,i'\in\mathcal{M}_f\},   \\
% 	u_{ii'} & \leftarrow \frac{\kappa}{\kappa + n_{ii'}},\quad \kappa>0,
% \end{align}
% 其中$\mathbb{I}(\cdot)$为指示函数，$n_{ii'}$ 累计了模型 $i$ 和 $i'$ 被一起包含于形式的次数，$\kappa$为初始虚拟观测次数。初始设定 $s_{ii'}=0,\ u_{ii'}=1,\ n_{ii'}=0$。随着二者共同评估次数增多，$u_{ii'}$ 单调减小，表示我们对其关系的信心提高。最后，对于每个参与当前形式的模型 $M_i\in \mathcal{M}_f$ 和任务 $\tau_j \in \mathcal{T}_f$，对模型--任务边的性能指标$\pi_{ij}$ 做指数滑动更新，用以校准 $M_i$ 在 $\tau_j$ 上的近期贡献。

% 综上所述，知识图谱是随优化过程动态演化的。初始时对任何模型关系都不了解，所以将所有 $s_{ii'}$ 设为0、$u_{ii'}$ 设为1。随着逐步评估各种形式，图谱记录的信息愈加丰富，所包含的知识也愈加准确。知识图谱在整个融合求解过程中扮演全局记忆库的角色，显式存储了各模型之间、模型与任务之间已经探明的关系，为后续未探明的组合提供指导依据。
...
% 采用节点类型特定的线性变换将初始嵌入映射至共同空间：
% \begin{equation}
% 	\mathbf{z}^{M,(0)}_i = W_M^{(0)} \mathbf{h}^M_i,\qquad
% 	\mathbf{z}^{T,(0)}_j = W_T^{(0)} \mathbf{h}^T_j,
% \end{equation}
% 其中 $W_M^{(0)}\in\mathbb{R}^{d'\times d}$，$W_T^{(0)}\in\mathbb{R}^{d'\times d_T}$。
% 随后堆叠 $L$ 层异构图注意力网络（GAT）\cite{velickovic2018graph}，
% 每层针对不同边类型采用独立参数。以模型节点 $i,i'$ 以及任务节点 $j$ 为例，
% 第 $r$ 层的注意力打分与聚合为：
% \begin{align}
% 	e^{(r)}_{i\leftarrow i'}         & = \mathrm{LeakyReLU}\!\left(\mathbf{a}_{\mathrm{MM}}^{(r)\top}
% 			[W_{\mathrm{att},M}^{(r)}\mathbf{z}^{M,(r)}_i\Vert
% 	W_{\mathrm{att},M}^{(r)}\mathbf{z}^{M,(r)}_{i'}\Vert s_{ii'}\Vert u_{ii'}]\right),                                       \\
% 	e^{(r)}_{i\leftarrow j}          & = \mathrm{LeakyReLU}\!\left(\mathbf{a}_{\mathrm{MT}}^{(r)\top}
% 			[W_{\mathrm{att},M}^{(r)}\mathbf{z}^{M,(r)}_i\Vert
% 	W_{\mathrm{att},T}^{(r)}\mathbf{z}^{T,(r)}_j\Vert \pi_{ij}]\right),                                                      \\
% 		\eta^{(r)}_{i\leftarrow \cdot} & = \mathrm{softmax}_{\cdot\in\mathcal{N}(i)}\!\left(e^{(r)}_{i\leftarrow\cdot}\right), \\
% 	\mathbf{z}^{M,(r+1)}_i           & = \mathrm{ELU}\!\left(
% 		\sum_{i'\in\mathcal{N}_{\mathrm{MM}}(i)}\!\eta^{(r)}_{i\leftarrow i'} W_{\mathrm{msg},M}^{(r)}\mathbf{z}^{M,(r)}_{i'}
% 		+\sum_{j\in\mathcal{N}_{\mathrm{MT}}(i)}\!\eta^{(r)}_{i\leftarrow j} W_{\mathrm{msg},T}^{(r)}\mathbf{z}^{T,(r)}_{j}
% 	\right).
% \end{align}

% 任务节点的更新方式与之相同。其中 $||$ 表示向量按特征维度的拼接。$W_{\mathrm{msg},M}^{(\ell)},\ W_{\mathrm{msg},T}^{(\ell)}$ 为消息变换矩阵。经过 $L$ 层传播后，
% 得到每个节点的高阶表示 $\mathbf{z}^M_i$（模型节点）和 $\mathbf{z}^T_j$（任务节点）。

% 为了得到针对某一候选形式 $f$ 的预测，取出该形式诱导的子图，即包含 $\mathcal{M}_f$ 中的模型节点及 $\mathcal{T}_f$ 中的任务节点，以及它们之间的所有连边。然后，在这个子图上应用三个并行的输出头进行汇聚：

% \textbf{(1) 性能预测头}：
% 输出标量 $\hat{y}_f$ 预测该形式在综合评价函数 $\mathcal{F}$ 下的得分。对模型与任务集合分别取均值池化并拼接，经 MLP 回归预测整体性能及对应损失为：
% \begin{gather}
% 	\hat{y}_f=\mathrm{MLP}\!\left(
% 	\frac{1}{|\mathcal{M}_f|}\sum_{i\in\mathcal{M}_f}\mathbf{z}^M_i\
% 	\Big\Vert\
% 	\frac{1}{|\mathcal{T}_f|}\sum_{j\in\mathcal{T}_f}\mathbf{z}^T_j
% 	\right),																					 \\
% 	\mathcal{L}_{\mathrm{perf}}=w_{\mathrm{perf}}\big(\hat{y}_f-y_f\big)^2.
% \end{gather}

% \textbf{(2) 权重预测头}：
% 输出形式 $f$ 中各模型的融合权重建议 $\hat{\boldsymbol{\alpha}}_f = (\hat{\alpha}_{f,i}: M_i \in \mathcal{M}_f)$。以 $\{\mathbf{z}^M_i:i\in\mathcal{M}_f\}$ 为输入序列，经过单层 Transformer 编码器\cite{attentionisall_vaswani_2017} 进行注意力聚合并经线性映射与 Softmax 得到融合权重及对应损失为：
% \begin{gather}
% 	\hat{\boldsymbol{\alpha}}_f=\mathrm{softmax}\!\big(W_\alpha\,\mathrm{AttnEnc}\{\,\mathbf{z}^M_i:i\in\mathcal{M}_f\,\}\big),																					 \\
% 	\mathcal{L}_{\alpha}=w_\alpha\,\mathrm{KL}\!\big(\boldsymbol{\alpha}^\star_f\ \|\ \hat{\boldsymbol{\alpha}}_f\big).
% \end{gather}

% \textbf{(3) 协同预测头}：
% 输出形式 $f$ 中各模型对的协同矩阵 $\hat{S}_f \in \mathbb{R}^{m\times m}$（$m=|\mathcal{M}_f|$）。将每个模型节点表示映射为 $h$ 维向量 $\mathbf{q}_i=W_q\mathbf{z}^M_i$，输出对称零对角矩阵及对应损失为：
% \begin{gather}
% 	(\hat{S}_f)_{ii'}=(1-\delta_{ii'})\,\tanh\!\big(\mathbf{q}_i^\top \mathbf{q}_{i'}\big),																													 \\
% 	\mathcal{L}_{\mathrm{syn}}=w_{\mathrm{syn}}\big\|\hat S_f-S^{\mathrm{KG}}_f\big\|_F^2.
% \end{gather}

% 图神经网络通过在线学习不断提高上述预测的准确性。每当一个形式 $f$ 被实际评估后，将其真实结果 $(y_f, \boldsymbol{\alpha}_f^\star)$ 作为训练样本，对 GNN 的参数进行更新。训练损失函数设计为同时兼顾三种预测的误差：
% \begin{equation}
% 	\mathcal{L}_{\text{GNN}} =
% 	w_{\text{perf}}\mathcal{L}_{\mathrm{perf}}
% 	+
% 	w_{\alpha}\mathcal{L}_{\alpha}
% 	+
% 	w_{\text{syn}}\mathcal{L}_{\mathrm{syn}}
% \end{equation}
% 其中 $w_{\text{perf}}, w_{\alpha}, w_{\text{syn}}$ 为超参数权重。通过最小化损失，GNN 学会将预测对齐至图谱中的已有知识和当前求解得到的新知识：KL 项确保 GNN 给出的融合配比逐渐贴近已知最优配比，协同项促使其内隐学习模型对的相互作用规律。随着不断有新形式解入图训练，GNN 的预测性能会持续提升，从而更好地支持形式选择与求解热启动。
...
本节详细阐述 KG-MFTO 的理论框架。该框架的核心是一个动态演化的知识图谱与一个图神经网络（Graph Neural Network，GNN）构成的闭环系统。知识图谱作为全局显式记忆，负责存储跨形式的知识；GNN 则对该图谱进行参数化推理，为未见形式提供先验知识。

具体而言，本节将首先定义动态异构知识图谱的结构（节点、边及其属性），随后介绍其边上统计量的确定性更新规则。最后，将详细设计 GNN 的结构及其训练目标。GNN 的输出（如性能预测 $\hat y_f$、权重建议 $\hat{\boldsymbol{\alpha}}_f$、协同矩阵 $\hat S_f$）将直接指导后续的课程规划与求解器热启动，实现知识的积累与利用。

\mysubsection{动态异构知识图谱建模}
\label{sec:ch5-4-1-dynamic-heterogeneous-kg-modeling}

本章工作构建一个动态异构图 $G = (V, E)$ 来统一表征模型融合问题中的关键实体与关系。

\textbf{节点定义}：图谱包含两类节点 $V = \mathcal{V}_M \cup \mathcal{V}_T$。
\begin{enumerate}
	\item \textbf{模型节点 ( $\mathcal{V}_M$)}：每个节点 $M_i \in \mathcal{V}_M$ 对应一个专家模型。其节点属性为可训练的 $d_M$ 维嵌入向量 $\mathbf{h}^M_i \in \mathbb{R}^{d_M}$，该向量在 GNN 训练过程中端到端优化。
	\item \textbf{任务节点 ( $\mathcal{V}_T$)}：每个节点 $\tau_j \in \mathcal{V}_T$ 对应一个下游任务。其节点属性为固定的 $d_T$ 维语义向量 $\mathbf{h}^T_j \in \mathbb{R}^{d_T}$。该向量通过预训练的 SimCSE 编码器 \cite{gao2021simcse} 从任务描述文本中提取，并在整个优化流程中保持冻结，为图谱提供任务层面的语义锚点。
\end{enumerate}

\textbf{边及属性定义}：图谱包含两类边 $E = \mathcal{E}_{MM} \cup \mathcal{E}_{MT}$，其边属性（统计量）随优化过程动态更新。
\begin{enumerate}
	\item \textbf{模型-模型边 ( $\mathcal{E}_{MM}$)}：$e_{ii'} = (M_i, M_{i'}) \in \mathcal{E}_{MM}$ 刻画模型间的交互关系。每条边维护两个实值统计量：协同分数 (Synergy Score) $s_{ii'} \in \mathbb{R}$ 和不确定度 (Uncertainty) $u_{ii'} \in [0, 1]$。$s_{ii'}$ 度量模型 $M_i$ 和 $M_{i'}$ 在融合时的协作效果：正值表示互补增益，负值表示冲突互斥。$u_{ii'}$ 表示当前对 $s_{ii'}$ 估计的不确定性或信息稀缺程度。
	\item \textbf{模型-任务边 ( $\mathcal{E}_{MT}$)}：$e_{ij} = (M_i, \tau_j) \in \mathcal{E}_{MT}$ 刻画模型与任务的适配性。每条边记录模型 $M_i$ 在任务 $\tau_j$ 上的历史性能指标 $\pi_{ij} \in \mathbb{R}$。
\end{enumerate}

\mysubsection{知识图谱动态更新机制}
\label{sec:ch5-4-2-kg-dynamic-update-mechanism}

知识图谱的边属性（统计量）在每次“形式”评估完成后进行确定性更新。假设当前评估了形式 $f=(\mathcal{M}_f, \mathcal{T}_f)$（其中 $\mathcal{M}_f \subseteq \mathcal{V}_M, \mathcal{T}_f \subseteq \mathcal{V}_T$），并通过求解器获得了（近）最优融合权重 $\boldsymbol{\alpha}^\star_f$ 及其对应的验证集性能 $y_f = \mathcal{F}(\mathrm{Merge}(\mathcal{M}_f, \boldsymbol{\alpha}^\star_f), \mathcal{T}_f)$。

\textbf{模型-模型边 ($\mathcal{E}_{MM}$) 更新}：
对于形式 $f$ 中任意共同出现的模型对 $(M_i, M_{i'}) \subseteq \mathcal{M}_f$，首先计算其在该形式下的协同增益 $g_f(i,i')$：
\begin{equation}
	g_f(i,i') = \Big(y_f - \bar{y}^{\text{ref}}_f\Big) \cdot \psi(\alpha^\star_{f,i}, \alpha^\star_{f,i'}), \quad \psi(a,b)=2ab,
\end{equation}
其中 $\bar{y}^{\text{ref}}_f$ 为 $\mathcal{M}_f$ 中单模型在 $\mathcal{T}_f$ 上的最佳性能（作为参考基线），$\psi(\cdot,\cdot)$ 项用于度量二者在 $\boldsymbol{\alpha}^\star_f$ 中的交互强度。
随后，采用指数滑动平均 (Exponential Moving Average, EMA) 更新协同分数 $s_{ii'}$：
\begin{equation}
	s_{ii'} \leftarrow (1-\lambda_s) s_{ii'} + \lambda_s g_f(i,i'),
\end{equation}
其中 $\lambda_s \in (0, 1)$ 为更新率。$s_{ii'}$ 通过累积不同形式下的 $g_f(i,i')$，逐渐收敛至全局协作关系。
不确定度 $u_{ii'}$ 则采用计数式退火 (Counter-based Annealing) 方式更新：
\begin{align}
	n_{ii'} & \leftarrow n_{ii'} + \mathbb{I}\{i,i'\in\mathcal{M}_f\}, \label{eq:ch5-counter} \\
	u_{ii'} & \leftarrow \frac{\kappa}{\kappa + n_{ii'}}, \label{eq:ch5-uncertainty}
\end{align}
其中 $\mathbb{I}(\cdot)$ 为指示函数，$n_{ii'}$ 累计模型对 $(i, i')$ 的共同评估次数，$\kappa > 0$ 为控制衰减速率的虚拟观测次数。

\textbf{模型-任务边 ($\mathcal{E}_{MT}$) 更新}：
对于当前形式中的每个模型 $M_i \in \mathcal{M}_f$ 和任务 $\tau_j \in \mathcal{T}_f$，同样使用 EMA 来更新其性能指标 $\pi_{ij}$，利用 $y_f$ 作为对 $\pi_{ij}$ 的新观测：
\begin{equation}
	\pi_{ij} \leftarrow (1-\lambda_\pi) \pi_{ij} + \lambda_\pi y_f,
\end{equation}
其中 $\lambda_\pi \in (0, 1)$ 为更新率。

\textbf{初始化}：在优化开始前，所有 $s_{ii'} \leftarrow 0$, $u_{ii'} \leftarrow 1$, $n_{ii'} \leftarrow 0$, $\pi_{ij} \leftarrow 0$。
综上所述，知识图谱 $G$ 随优化过程动态演化。随着评估的形式增多，图谱中存储的 $s_{ii'}$ 将更准确地反映模型间的内在（互补或冲突）关系，$u_{ii'}$ 将降低，$\pi_{ij}$ 将更准确地反映模型在任务上的综合表现。

\mysubsection{图神经网络的关系推理}
\label{sec:ch5-4-3-gnn-relational-reasoning}

为利用知识图谱 $G$ 中积累的知识，本文设计了一个异构 GNN，对 $G$ 进行端到端的关系推理。GNN 的目标是预测任一候选形式 $f=(\mathcal{M}_f, \mathcal{T}_f)$ 的关键属性，从而指导上层决策。

\textbf{GNN 输入层}：
GNN 以 $G$ 的节点属性为输入。首先通过节点类型特定的线性变换，将不同维度的初始嵌入 $\mathbf{h}^M_i \in \mathbb{R}^{d_M}$ 和 $\mathbf{h}^T_j \in \mathbb{R}^{d_T}$ 映射至 $d'$ 维的共同隐空间：
\begin{equation}
	\mathbf{z}^{M,(0)}_i = W_M^{(0)} \mathbf{h}^M_i,\qquad
	\mathbf{z}^{T,(0)}_j = W_T^{(0)} \mathbf{h}^T_j,
\end{equation}
其中 $W_M^{(0)}\in\mathbb{R}^{d'\times d_M}$ 和 $W_T^{(0)}\in\mathbb{R}^{d'\times d_T}$ 是可训练参数。

\textbf{GNN 传播层}：
随后，堆叠 $L$ 层异构图注意力传播层\cite{velickovic2018graph}。在第 $r$ 层 ($0 \le r < L$)，节点表示 $\mathbf{z}^{(r)}$ 通过聚合邻域信息进行更新。对不同类型的边（MM, MT, TM）使用独立的参数。对于模型节点 $M_i$，其注意力分数 $e^{(r)}$ 计算方式如下（以 $M_{i'}$ 和 $\tau_j$ 为邻居为例）：
\begin{align}
	e^{(r)}_{i\leftarrow i'} & = \mathrm{LeakyReLU}\!\left(\mathbf{a}_{\mathrm{MM}}^{(r)\top}
			[W_{\mathrm{att},M}^{(r)}\mathbf{z}^{M,(r)}_i \Vert
	W_{\mathrm{att},M}^{(r)}\mathbf{z}^{M,(r)}_{i'} \Vert s_{ii'} \Vert u_{ii'}]\right), \label{eq:ch5-att-mm} \\
	e^{(r)}_{i\leftarrow j}  & = \mathrm{LeakyReLU}\!\left(\mathbf{a}_{\mathrm{MT}}^{(r)\top}
			[W_{\mathrm{att},M}^{(r)}\mathbf{z}^{M,(r)}_i \Vert
				W_{\mathrm{att},T}^{(r)}\mathbf{z}^{T,(r)}_j \Vert \pi_{ij}]\right), \label{eq:ch5-att-mt}
\end{align}
其中 $W_{\mathrm{att},\cdot}^{(r)}$ 为注意力变换矩阵，$\mathbf{a}_{\cdot}^{(r)}$ 为注意力向量，$\Vert$ 表示特征拼接。将边属性 $s_{ii'}, u_{ii'}, \pi_{ij}$ 作为额外特征融入注意力计算，为 GNN 提供了丰富的归纳偏置。

归一化的注意力系数 $\eta^{(r)}$ 通过 $\mathrm{softmax}$ 获得：
\begin{equation}
	\eta^{(r)}_{i\leftarrow k} = \frac{\exp(e^{(r)}_{i\leftarrow k})}{\sum_{k' \in \mathcal{N}(i)} \exp(e^{(r)}_{i\leftarrow k'})}, \quad k \in \mathcal{N}(i) = \mathcal{N}_{\mathrm{MM}}(i) \cup \mathcal{N}_{\mathrm{MT}}(i).
\end{equation}

模型节点 $M_i$ 的第 $r+1$ 层表示 $\mathbf{z}^{M,(r+1)}_i$ 通过加权聚合邻居消息得到：
\begin{equation}
	\mathbf{z}^{M,(r+1)}_i = \mathrm{ELU}\!\left(
	\sum_{i'\in\mathcal{N}_{\mathrm{MM}}(i)}\!\eta^{(r)}_{i\leftarrow i'} W_{\mathrm{msg},M}^{(r)}\mathbf{z}^{M,(r)}_{i'}
	+\sum_{j\in\mathcal{N}_{\mathrm{MT}}(i)}\!\eta^{(r)}_{i\leftarrow j} W_{\mathrm{msg},T}^{(r)}\mathbf{z}^{T,(r)}_{j}
	\right). \label{eq:ch5-agg-m}
\end{equation}
其中 $W_{\mathrm{msg},\cdot}^{(r)}$ 为消息变换矩阵。

类似地，对于任务节点 $\tau_j$，其邻居只包含模型节点 $\mathcal{N}(j) = \mathcal{N}_{\mathrm{TM}}(j)$。其更新方式对称定义如下：
\begin{align}
	e^{(r)}_{j\leftarrow i}    & = \mathrm{LeakyReLU}\!\left(\mathbf{a}_{\mathrm{TM}}^{(r)\top}
			[W_{\mathrm{att},T}^{(r)}\mathbf{z}^{T,(r)}_j \Vert
	W_{\mathrm{att},M}^{(r)}\mathbf{z}^{M,(r)}_i \Vert \pi_{ij}]\right),                                                        \\
	\eta^{(r)}_{j\leftarrow i} & = \mathrm{softmax}_{i \in \mathcal{N}_{\mathrm{TM}}(j)}\!\left(e^{(r)}_{j\leftarrow i}\right), \\
	\mathbf{z}^{T,(r+1)}_j     & = \mathrm{ELU}\!\left(
	\sum_{i\in\mathcal{N}_{\mathrm{TM}}(j)}\!\eta^{(r)}_{j\leftarrow i} W_{\mathrm{msg},M}^{(r)}\mathbf{z}^{M,(r)}_{i}
	\right). \label{eq:ch5-agg-t}
\end{align}
经过 $L$ 层传播后，得到高阶节点表示 $\mathbf{z}^M_i = \mathbf{z}^{M,(L)}_i$ 和 $\mathbf{z}^T_j = \mathbf{z}^{T,(L)}_j$。

\textbf{输出预测头 (Output Prediction Heads)}：
给定一个候选形式 $f=(\mathcal{M}_f, \mathcal{T}_f)$ 首先提取其诱导子图 $G_f$ 上的节点高阶表示 $\{\mathbf{z}^M_i\}_{i\in\mathcal{M}_f}$ 和 $\{\mathbf{z}^T_j\}_{j\in\mathcal{T}_f}$。然后，应用三个并行的输出头进行汇聚 (Readout)，以预测形式 $f$ 的不同属性：

\textbf{(1) 性能预测头}：
预测该形式的融合性能得分 $\hat{y}_f$。对模型和任务节点集分别应用均值池化，拼接后通过 MLP 回归：
\begin{gather}
	\hat{y}_f=\mathrm{MLP}_{\mathrm{perf}}\!\left(
	\mathrm{Mean}\!\left(\{\mathbf{z}^M_i\}_{i\in\mathcal{M}_f}\right)
	\ \Big\Vert\
	\mathrm{Mean}\!\left(\{\mathbf{z}^T_j\}_{j\in\mathcal{T}_f}\right)
	\right), \\
	\mathcal{L}_{\mathrm{perf}}=\big(\hat{y}_f-y_f\big)^2.
\end{gather}

\textbf{(2) 权重预测头}：
预测该形式的最优融合权重 $\hat{\boldsymbol{\alpha}}_f = (\hat{\alpha}_{f,i}: M_i \in \mathcal{M}_f)$，用于求解器热启动。将模型节点表示集 $\{\mathbf{z}^M_i\}_{i\in\mathcal{M}_f}$ 视为一个序列，输入至单层 Transformer 编码器 \cite{attentionisall_vaswani_2017} 以捕捉内部依赖，随后经线性层和 Softmax 得到权重：
\begin{gather}
	\hat{\boldsymbol{\alpha}}_f=\mathrm{softmax}\!\big(W_\alpha\,\mathrm{AttnEnc}\!\left(\{\mathbf{z}^M_i\}_{i\in\mathcal{M}_f}\right)\big), \\
	\mathcal{L}_{\alpha}=\mathrm{KL}\!\big(\boldsymbol{\alpha}^\star_f\ \|\ \hat{\boldsymbol{\alpha}}_f\big).
\end{gather}
使用 KL 散度损失，促使 GNN 预测的权重分布 $\hat{\boldsymbol{\alpha}}_f$ 逼近真实的最优权重分布 $\boldsymbol{\alpha}^\star_f$。

\textbf{(3) 协同预测头}：
预测形式 $f$ 内部 $m=|\mathcal{M}_f|$ 个模型间的协同矩阵 $\hat{S}_f \in \mathbb{R}^{m\times m}$。通过内积解码器生成对称且零对角的协同矩阵：
\begin{gather}
	\mathbf{q}_i=W_q\mathbf{z}^M_i, \quad (\hat{S}_f)_{ii'}=(1-\delta_{ii'})\,\tanh\!\big(\mathbf{q}_i^\top \mathbf{q}_{i'}\big), \\
	\mathcal{L}_{\mathrm{syn}}=\big\|\hat S_f-S^{\mathrm{KG}}_f\big\|_F^2.
\end{gather}
其中 $W_q$ 为可训练映射，$S^{\mathrm{KG}}_f \in \mathbb{R}^{m\times m}$ 为形式 $f$ 中模型对在知识图谱上对应的协同分数子矩阵，即 $(S^{\mathrm{KG}}_f)_{ii'} = s_{ii'}$。此损失促使 GNN 学习并内化 KG 中显式存储的模型交互关系。

\textbf{GNN 训练}：
每当一个形式 $f$ 被实际评估后，其结果 $(y_f, \boldsymbol{\alpha}^\star_f)$ 以及从 KG 中提取的 $S^{\mathrm{KG}}_f$ 将构成一个训练样本，用于 GNN 参数的在线更新。总损失函数 $\mathcal{L}_{\text{GNN}}$ 为上述三项损失的加权和：
\begin{equation}
	\mathcal{L}_{\text{GNN}} =
	w_{\mathrm{perf}}\mathcal{L}_{\mathrm{perf}}
	+
	w_{\alpha}\mathcal{L}_{\alpha}
	+
	w_{\mathrm{syn}}\mathcal{L}_{\mathrm{syn}},
\end{equation}
其中 $w_{\mathrm{perf}}, w_{\alpha}, w_{\mathrm{syn}}$ 为各项损失的超参数权重。通过最小化 $\mathcal{L}_{\text{GNN}}$，GNN 不断学习将图谱结构与统计知识（输入）映射到融合性能、最优配置与交互关系（输出）。
...
\begin{figure}
	...
	\bicaption[KG-MFTO算法框架]{KG-MFTO算法框架示意图。KG-MFTO通过动态知识图谱和图神经网络实现跨形式的知识共享与迁移，课程规划器负责生成和选择候选形式，演化求解器在每个形式上高效搜索最优融合配比。整个过程形成一个闭环，不断积累和利用经验以提升融合性能。知识图谱中，红色为模型节点，蓝色为任务节点。}[Framework of KG-MFTO]{Illustration of the KG-MFTO algorithm framework. KG-MFTO enables cross-modal knowledge sharing and transfer through dynamic knowledge graphs and graph neural networks. A curriculum planner generates and selects candidate modalities, while an evolutionary solver efficiently searches for the optimal merging ratios within each modality. The entire process forms a closed loop that continuously accumulates and leverages experience to enhance merging performance. In the knowledge graph, red represents model nodes, while blue denotes task nodes.}\label{fig:ch5-kg-mfto-framework}
\end{figure}
...
给定当前课程级别 $\ell$，期望产生若干候选形式，每个形式包含 $\ell$ 个模型。为了高效探索形式空间，采用混合启发的随机生成策略。本文默认超参数：$p_{\text{unc}}=0.3$、$p_{\text{syn}}=0.5$、候选池大小 $K=12$。在生成每一个候选形式时，按下列三种策略的给定概率选择其一：
...
此外，为确保跨任务指标的一致性与可比性，本文在实验中固定采用 $\mathcal{T}_f \equiv \mathcal{T}$（即四个基准任务全覆盖）进行规划与评估；聚焦式任务子集可作为扩展策略，但非本文默认设置。
...
\mysubsubsection{形式评分与选择}

一旦得到候选形式集 $\mathcal{C}_\ell$，利用上一节训练好的 GNN 对每个候选形式 $f\in\mathcal{C}_\ell$ 进行预测，并据此计算一个评分 $A(f)$ 来衡量其价值。本文采用类上置信界（Upper Confidence Bound，UCB）的评分策略\cite{auer2002finite}：
\begin{equation}
	A(f) = \hat{y}_f + \beta_b\,\mathrm{unc}(f),
\end{equation}
其中 $\hat{y}_f$ 是 GNN 性能头预测的形式 $f$ 综合得分，$\mathrm{unc}(f)$ 量化该形式的不确定性。本文将 $\mathrm{unc}(f)$ 定义为模型对不确定度集合 $\{u_{ii'}: i,i'\in \mathcal{M}_f\}$ 的截断均值（去除上下各 $10\%$ 的极值后求均值），以稳健代表 $f$ 内部关系知识的未知程度；探索系数按 $\beta_b = \beta_0 / \sqrt{1+b}$ 递减，默认 $\beta_0=1.0$。通过 UCB 评分，$\hat{y}_f$ 高或 $\mathrm{unc}(f)$ 高的形式均可获得较高分数，前者偏向性能利用，后者鼓励知识获取；随着 $b$ 增加，评分逐步偏向利用。

计算出所有候选的 $A(f)$ 后，课程规划器从 $\mathcal{C}_\ell$ 中选出得分最高的形式 $f^\star = \arg\max_{f\in\mathcal{C}_\ell} A(f)$ 作为本次迭代的目标形式。随后，系统将把 $f^\star$ 交由融合求解器进行优化。同时设置确定的课程级别提升规则：记当前级别下共同观察过的模型对不确定度集合为 $\mathcal{U}_\ell=\{u_{ii'}\}$，其截断均值为 $\bar{u}_\ell$（上下各截断 $10\%$）。当且仅当满足两条件时提升 $\ell\!\leftarrow\!\ell+1$：
（i）$\bar{u}_\ell \le u_{\text{thr}}$，默认 $u_{\text{thr}}=0.4$；
（ii）至少 $\rho$ 比例的模型对满足 $n_{ii'}\ge n_{\min}$，默认 $\rho=0.8,\ n_{\min}=2$。
该规则避免在低级别停留过久，同时确保在进入更高难度前已具备足够的关系认知。
...
% 若融合允许权重取正负（如对参数增量进行叠加的情形），采用 tanh 参数化。$\alpha_i = \frac{\tanh(z_i)}{\sum_{i'}|\tanh(z_{i'})|}$，这样 $z_i$ 任意实数均可通过 $\tanh$ 压缩在 $(-1,1)$，再归一化保证和为 1 的绝对值。直观上，这是对正负贡献进行归一的表示。本文实验统一采用 Softmax 参数化；tanh 参数化作为允许负权重的扩展选项，不在默认设置中启用。
...
形式 $f^\star$ 求解完成后，KG-MFTO 进入学习步骤，即根据新获得的成果更新知识图谱和 GNN，从而完成一次闭环。具体步骤如下：首先，将形式 $f^\star$ 的解 $\boldsymbol{\alpha}^\star_{f^\star}$ 及其性能 $y_{f^\star}$ 用于更新知识图谱中的边属性：对于 $f^\star$ 中每一对模型 $(M_i,M_{i'})$，计算 $g_{f^\star}(i,i')=(y_{f^\star}-\bar{y}^{\text{ref}}_{f^\star})\cdot 2\alpha^\star_{f^\star,i}\alpha^\star_{f^\star,i'}$ 并相应地更新协同分数 $s_{ii'}$ 以及观测次数 $n_{ii'}$ 和不确定度 $u_{ii'}$；同时，对于 $f^\star$ 涉及的每个模型和任务，采用指数滑动方式更新模型--任务性能 $\pi_{ij}$，以反映近期融合在任务 $\tau_j$ 上的贡献强度。接着，以 $(f^\star,\boldsymbol{\alpha}^\star_{f^\star},y_{f^\star})$ 作为新的训练样本，对图神经网络执行数步梯度下降以最小化损失，更新 GNN 参数，使其能够重现该形式的结果。通过这一步，知识图谱和代理模型都更为智能。图谱中显式存储了新的模型关系数据，GNN 则隐式将这些数据泛化为对其它潜在形式的更新预测。

完成学习后，检查课程级别是否需要调整。如前所述，若当前级别的形式探索已达到预设充分度且尚未达到最大级别，则令 $\ell \leftarrow \ell+1$，以便下次迭代生成更大规模的形式来求解。随后进入下一循环。整个闭环将持续运行直到耗尽预设的评估预算 $B$（即最多评估 $B$ 个形式的性能），或达到其它终止条件（如已找到全局融合的满意解）。最终，知识图谱中应已存有关于各模型关系的丰富知识，并获得了一系列所求解过的形式及其融合方案，其中最高级别，即全体模型的融合方案即为最终目标所需的结果。算法 \ref{alg:kg-mfto} 综合展示了 KG-MFTO 的完整流程。结果显示，课程规划、求解和图谱更新彼此交替，构成了一个自适应的闭环优化算法。
...
\begin{algorithm}[tb]
	...
	初始化知识图谱 $G$（节点包括$\mathcal{M}$和$\mathcal{T}$，边协同 $s_{ii'}=0$，不确定度 $u_{ii'}=1$ 等）；\ 初始化 GNN 参数；\\
	...
	\While{$b < B$}{
		...
		\For{\textbf{each} $f \in \mathcal{C}_\ell$}{
			...
			计算 $\displaystyle \mathrm{unc}(f) = \mathrm{TrimMean}_{10\%}\big(\{u_{ii'}: i,i'\in\mathcal{M}_f\}\big)$；\\
			计算 $A(f) = \hat{y}_f + \beta_b\,\mathrm{unc}(f)$ ；
		}
		...
		令 $b\leftarrow b+1$；
		\If{$\mathrm{TrimMean}_{10\%}(\{u_{ii'}\}_{\text{level }\ell}) \le u_{\text{thr}}$ \textbf{ and } $\mathrm{Frac}(\{n_{ii'}\ge n_{\min}\}_{\text{level }\ell}) \ge \rho$}{
			$\ell \leftarrow \ell + 1$；
		}
	}
...
选取 LLaMA-3.1-8B-Instruct 模型作为统一的基础模型（基座模型），并构建七个在不同领域数据上微调的专家模型（记为 M1--M7）。这些模型通过相同的 SFT 过程从基础模型 fine-tune 而来，只是训练数据各不相同，以确保模型结构与规模一致、差异仅来源于所习得的知识。表 列出了七个模型的领域及微调语料概况，包括：M1（英文指令执行），M2（中文问答），M3（数学解题），M4（代码生成），M5（常识推理），M6（科学问答）和 M7（对齐/安全）。这样选择旨在覆盖广泛而互补的能力范围。将所有这七个模型进行融合，以模拟一个复杂而具有高干扰风险的情形，同时挑选四类具有代表性的任务来评价最终融合模型的性能：
...
以上四项任务各自对应专家模型中的某一领域（例如 M4 专长代码，M3 专长数学等），也有交叉：如 M1 泛指令模型对所有有帮助，M2 中文知识模型对 CMMLU\cite{li2023cmmlu} 有贡献等等。因此，能否融合出在所有任务上均表现优异的模型，是对本章方法的严格考验。在每项任务上都有各自的测试集用于最终评估，在融合过程中则另准备了独立的验证集供算法评估使用。为了量化融合效果，引入性能保留率（Performance Retention Rate，PRR）指标：对于任务 $t$，有
\begin{equation}
	\text{PRR}(t) = \frac{S_{\text{merged}}(t)}{S_{\text{finetune}}(t)} \times 100\%,
\end{equation}
其中 $S_{\text{merged}}(t)$ 为融合模型在任务 $t$ 上的得分，$S_{\text{finetune}}(t)$ 则是对应领域的专家模型单独在该任务上的得分（即视各专家模型为上限，其自身在相应领域的能力为 100\%）。PRR 衡量融合模型相对于专家模型的能力保留程度。主要关注平均 PRR（四个任务的均值）作为整体指标，同时报告每个任务上的 PRR 及原始得分。
...
\begin{table}[tb]
	...
	\small\begin{tabular}{ll}
		...
		优化方法         & AdamW（学习率 \(1\times10^{-3}\)，权重衰减 \(1\times10^{-5}\)） \\
...
上述基线涵盖了从静态平均、启发式参数操作到自适应优化的多种融合范式。所有方法均在融合全部 \( N = 7 \) 个模型后，于各任务上评估其性能并计算 PRR。本文中的 $\textrm{Merge}$ 算子均采用 TIES-Merging，以确保融合过程中的参数对齐和稳定性。对于 KG-MFTO，本章方法设置评估预算 \( B = 100 \)。CMA-ES 求解器的种群大小设为 10，最大迭代代数为 30，停滞重启代数 \( G_{\text{stall}} = 5 \)。这些设置在实验前通过小规模预实验进行调优，以平衡性能与计算成本。
...
图 \ref{fig:perf-vs-eval-count} 展示了在单次融合优化过程中，EOMMR 与 KG-MFTO 的性能收敛曲线。横轴为 CMA-ES 候选解的评估次数，纵轴为归一化后平均得分（0–100）。结果显示，EOMMR 作为标准演化搜索，性能提升相对缓慢，约在 200 次评估后达到 69.6 分并趋于饱和；而 KG-MFTO 在相同条件下实现了显著加速，仅 70 次评估便能超过 69 分，并在 120 次左右达到 71 分的稳定上限。
...
为探究知识迁移机制在优化过程中发挥的作用，记录了 KG-MFTO 在不断求解新形式时，知识图谱质量和算法性能随经验积累的变化。如表所示，将总评估次数 $B$ 分别取 25、50、100，统计此时知识图谱中模型--模型协同预测与实际最优权重之间的 Spearman 秩相关系数 $r_s$，以及算法在解决一个全新融合问题，即未见过的形式时直接使用 GNN 建议解的未经优化的启动性能。结果显示，随着算法求解的形式增多，知识图谱对模型关系的把握显著提升：$r_s$ 从 25 次后的 0.31 提高到 100 次后的 0.62，表明图谱中学到的协同分数与实际观测到的模型重要性排序越来越一致。同时，不确定度随着观测次数增加而降低，反映出图谱知识变得更可靠。更直观的是，算法在未优化时就能给出的初始宏观得分逐步攀升，从 68.5 增长至 73.9。这说明，通过反复的知识积累和 GNN 学习，KG-MFTO 学会了如何基于以往经验对新问题做出较好决策，使其对融合的驾驭能力越来越强。这种优化过程中的自我蒸馏现象充分证明了知识迁移的价值：在解决主问题的同时，也在训练一个代理模型，使其渐渐接近主问题的解分布，从而实现优化的经验重用和收益递增。

\begin{table}[tb]
	\centering
	\bicaption[求解形式数量对知识图谱的影响]{知识图谱质量随求解形式数量的提升趋势与饱和验证。$r_s$ 表示知识图谱预测的协同值与最终融合权重之间的 Spearman 相关系数；启动得分为无需搜索、直接采用知识图谱/GNN 建议配方时的归一化平均得分（0–100）。}[The Impact of The Number of Forms Solved on Knowledge Graphs]{Trend and saturation verification of knowledge-graph quality with increasing number of solved forms. $r_s$ denotes the Spearman correlation between predicted synergy scores and final merging weights; the startup score reports the normalized overall score (0–100) achieved by directly adopting the KG/GNN-suggested merging without search.}
	...
	\begin{tabular}{ccccc}
		...
		\midrule
		$r_s$(预测协同, 最优权重) & 0.31 & 0.47 & 0.62 & 0.63 \\
...
综上，实验结果全面验证了本章提出方法在多模型融合任务上的有效性。KG-MFTO不仅显著提升了融合模型的性能，使其在多个任务上达到接近专家模型的水准；更为重要的是，它成功解决了随着融合规模扩大而出现的效率瓶颈与冲突难题，实现了可扩展、高效的融合优化。这一成果证明了知识迁移与课程优化思想在复杂模型构建问题上的巨大潜力。
...
本章的方法为深度模型的模块化复用提供了一种新思路，即通过分解和迁移来解决整体优化难题，避免一步到位的暴力过程。这一思路不仅适用于 LLM 的融合，对于其他需要组合多个已训练组件的场景（例如多模态模型融合、专家系统集成等）也有潜在借鉴意义。
